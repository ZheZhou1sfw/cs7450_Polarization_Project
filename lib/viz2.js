...
var globalYearG = viz2mainG.append('g')
    .attr('class', 'title');
...
.then(function(dataset) {
    ...
    sliderTime = d3
        ...
        .on('onchange', val => {
            ...
            var secondTextElem = globalYearG.select('text');
            secondTextElem.transition().duration(500).text(function() {
                return ((globalCong - 97) * 2 + 1981).toString();
            });
...
d3.selectAll('.toggleButtons')
    .on('click', function() {
        ...
        document.getElementsByClassName('toggleButtons')[0].style.visibility = 'hidden';
    });
